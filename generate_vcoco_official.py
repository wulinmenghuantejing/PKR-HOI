...
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1])['hoi'][0]
...
def get_args_parser():
    ...
    parser.add_argument('--cnn_layers', default=1, type=int,
...
def main(args):
    ...
    checkpoint = torch.load(args.param_path, map_location=torch.device('cpu'))
    model.load_state_dict(checkpoint['model'])

    detections = generate(model, post_processor, data_loader_val, device, verb_classes, args.missing_category_id)


    if args.save_path.endswith('pickle'):
        with open(args.save_path, 'wb') as f:
            pickle.dump(detections, f, protocol=2)
    elif args.save_path.endswith('torch'):
        torch.save(detections, args.save_path)
