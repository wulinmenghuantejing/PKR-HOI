...
class DETRHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False):
        ...
        # detr的类别和坐标
        self.class_embed = nn.Linear(hidden_dim, num_obj_classes + 1)
    ...
    def forward(self, samples: NestedTensor):
        ...
        hss = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1])
        hs_detr = hss['detr'][0]
        hs = hss['hoi'][0]
        verbs = hss['verbs']
        ...
        if torch.isnan(outputs_sub_coord[-1,:,10,:]).sum() > 0:
            print("nan exists")
            print('log_outputs_sub_coord',outputs_sub_coord[-1,:,:10,:])
            print('log_hs',hs[-1,:,:3,:])
            exit()
        outputs_obj_coord = self.obj_bbox_embed(hs).sigmoid()
        # DETR
        outputs_class = self.class_embed(hs_detr)
        outputs_coord = self.bbox_embed(hs_detr).sigmoid()
        out_detr = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]}
        if self.aux_loss:
            out_detr['aux_outputs'] = self._set_aux_loss_detr(outputs_class, outputs_coord)
        # HOI
        ...
        return {'detr':out_detr, 'hoi':out, 'verbs':verbs}
    ...
    @torch.jit.unused
    def _set_aux_loss_detr(self, outputs_class, outputs_coord):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
